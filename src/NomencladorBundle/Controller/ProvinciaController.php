...
use NomencladorBundle\Entity\Provincia;

/**
 * @Route("nomenclador/provincia/")
 */
class ProvinciaController extends Controller
{
    /**
     * List All values from Provincia
     *
     * @Route("list")
     */
    public function listAction()
    {
        ...
        foreach($this->getDoctrine()->getManager()->getRepository('NomencladorBundle:Provincia')->findAll() as $value ) {
            $data[] = array(
                'id' => $value->getId(), 
    ...
    public function addEditAction(Request $rq)
    {
        ...
        /* Add or Edit Provincia */
        if ($rq->get('id')) {
            $entity = $em->getRepository('NomencladorBundle:Provincia')->find($rq->get('id'));
        } else {
    ...
    public function removeAction(Request $rq)
    {
        ...
        foreach (json_decode($rq->get('ids')) as $id) {
            $entity = $em->getRepository('NomencladorBundle:Provincia')->find($id);
            $em->remove($entity);
        }
        return new Response($em->flush());
    }
